...
object JsonMap {
  ...
  given anyCodec: JsonValueCodec[Any] =
    new JsonValueCodec[Any] {
      def decodeValue(in: JsonReader, default: Any): Any =
        in.nextToken() match {
          case 'n' =>
            if (in.nextByte() == 'u' && in.nextByte() == 'l' && in.nextByte() == 'l') null
          ...
          case 't' =>
            if (in.nextByte() == 'r' && in.nextByte() == 'u' && in.nextByte() == 'e') true
  ...
  given jsonMapCodec: JsonValueCodec[JsonMap] =
    new JsonValueCodec[JsonMap] {
