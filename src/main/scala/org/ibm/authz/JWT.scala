...
object JwtConfig:
  def fromEnvironmentBase64(): Try[JwtConfig] =
    Try {
...
class WxoJwtService(config: JwtConfig):
  ...
  /** Creates a JWT token. This method shows EXACTLY what goes into the JWT. Read this top-to-bottom
    * to understand the token structure.
    */

  def createJwtToken(
      userInfo: UserInfo,
      ssoToken: String
  ): cats.effect.IO[Option[String]] = {
          ...
          val claim = JwtClaim(
            ...
            expiration = Some(
              x._3
                ...
                .getOrElse(
                  System
                    .currentTimeMillis() + sys.env.get("TOKEN_EXPIRY").map(_.toInt).getOrElse(600)
                )
            )
          )
