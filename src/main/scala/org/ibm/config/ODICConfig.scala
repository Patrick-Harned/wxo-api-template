...
object OIDCConfig {
  def fromEnv: OIDCConfig =
    OIDCConfig(
      ...
      introspectionEndpoint = sys.env.getOrElse("OIDC_INTROSPECTION_ENDPOINT", ""),
      redirectUri = sys.env.getOrElse("OIDC_REDIRECT_URI", "http://localhost:8080/callback"),
