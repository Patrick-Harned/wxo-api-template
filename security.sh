...
if [[ "${IS_CPD}" == "true" ]]; then
  ...
  RESPONSE=$(curl -X POST 'https://iam.cloud.ibm.com/identity/token' -H 'Content-Type: application/x-www-form-urlencoded' -d "grant_type=urn:ibm:params:oauth:grant-type:apikey&apikey=${WXO_API_KEY}" )
  echo $RESPONSE
  WXO_TOKEN=$(echo $RESPONSE | jq -r .access_token)
  if [[ -z "${WXO_TOKEN}" || "${WXO_TOKEN}" == "null" ]]; then
